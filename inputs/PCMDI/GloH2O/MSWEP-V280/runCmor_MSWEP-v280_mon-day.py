...
mos = ['01']
if freq == 'A3hr': mos = ['01','02','03','04','05','06','07','08','09','10','11','12']
...
#  yr = yr  # + '00*' #day    # '001 for 3hr'
  ...
  if freq == 'A3hr': fc = xc.open_mfdataset(pathind, mask_and_scale=False, decode_times=True, combine='nested', concat_dim='time', preprocess=extract_date, data_vars='all')
...
#  tunits = "hours since 1900-1-1 00:00:00"  #fc.time.units
  ...
  for mo in mos:
   fcc = fc
   if freq == 'A3hr': fcc = fc.sel(time=slice(yr + "-" + mo,yr + "-" + mo))

   tdc = fcc.time.values[:]
   if freq != 'A3hr': tdc = tdc.astype(float) 
...
#cmor.set_cur_dataset_attribute('history',f.history) ; # Force input file attribute as history
   ...
   cmorLat = cmor.axis("latitude", coord_vals=lat.values, cell_bounds=fcc.lat_bnds.values, units="degrees_north")
   cmorLon = cmor.axis("longitude", coord_vals=lon.values, cell_bounds=fcc.lon_bnds.values, units="degrees_east")
