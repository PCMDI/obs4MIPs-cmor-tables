...
2) _*Retrieve data desired to be made obs4MIPs compliant*_.  To accomplish this, typically a dataset is downloaded from the internet before beginning the process.  The examples on this repo assume the data downloaded is in netCDF format, but it does not have to be.  In general, obs4MIPs data products are prepared from publically available (but not obs4MIPs compliant) data from via the official curators of the data or recognized data centers.     
