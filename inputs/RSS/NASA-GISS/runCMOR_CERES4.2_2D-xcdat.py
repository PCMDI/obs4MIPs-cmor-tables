...
for fi in range(len(inputVarName)):
  ...
  inputFilePath = 'CERES_EBAF4.2/' # change to user's path where file is stored
...
# Open and read input netcdf file
  ...
  # quick fix of datumyr parsing and of unpacking count mismatch 
  since_str = time.units.split("since", 1)[1].strip()   # e.g. "2000-03-01 00:00:00"
  datumyr = int(since_str[0:4])                         # 2000
  datummnth = int(since_str[5:7])                       # 03
  ret = monthly_times(datumyr, years, datum_start_month=datummnth, start_month=start_month, end_month=end_month)
  time_new, time_bounds_new = ret[0], ret[1]   # ignore anything else
...
# gitinfo = obs4MIPsLib.ProvenanceInfo(obs4MIPsLib.getGitInfo("./"))
  ...
  gitinfo = obs4MIPsLib.ProvenanceInfo(obs4MIPsLib.getGitInfo(script_file))
