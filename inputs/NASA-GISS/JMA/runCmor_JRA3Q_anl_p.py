...
inputFilePath = '/global/cfs/projectdirs/m4581/obs4MIPs/obs4MIPs_input/JMA/JRA3Q_monthly/monthly3D'
inputVarName = [v + '-pres-an-gauss-mn' for v in ['tmp','ugrd','vgrd','hgt','spfh']]
outputVarName = ['ta','ua','va','zg','hus']  
outputUnits = ['K','m s-1','m s-1','m','1'] 
valid_min=[150.,-200.,-200.,-500.,0.]
valid_max=[350.,200.,200.,100000.,0.05]
run_version = "v" + datetime.now().strftime("%Y%m%d") # fixed for entire run
cmor_missing = np.float32(1.0e20)

#seperate the whole data period into 4 subsets to limit output file size
periods = [
    ("1947", "1969"),
    ("1970", "1989"),
    ("1990", "2009"),
...
for var, out_var, out_unit, out_min, out_max in zip(inputVarName, outputVarName, outputUnits,valid_min,valid_max):
    allFiles = sorted(glob.glob(f"{inputFilePath}/{var}/*.{var}.jra3q-ms-mn.anl_p.*.{var}.??????0100_????????18.nc"))
    if len(allFiles) == 0:
        continue
    ...
    for start_year, end_year in periods:
        ...
        inputFiles = [
            ...
            if start_year <= os.path.basename(fn).split('.')[-2][:4] <= end_year
        ]
        ...
        lev = f.pressure_level.values*100. # convert to Pa
        ...
        axes = [
            ...
            {"table_entry": "plev45-JRA3Q", "units": "Pa",
             "coord_vals": lev},
        ...
        varid = cmor.variable(out_var + '-plev45',out_unit,axisIds,missing_value=cmor_missing)
