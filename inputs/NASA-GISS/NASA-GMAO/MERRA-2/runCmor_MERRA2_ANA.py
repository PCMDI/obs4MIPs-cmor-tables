...
for year in range(1980, 2027):  # put the years you want to process here
    ...
    inputFiles.sort(key=lambda f: datetime.strptime(f.split('.')[2], "%Y%m"))
    ...
    lev = f.lev.values*100. # convert to Pa
    ...
    for fi in range(len(inputVarName)): #looping over varaibles
        ...
        values = np.where(mask, cmor_missing, values) #convert to kg m-2 s-1
        ...
        f.close()
        cmor.close()
        print(f"File written for {fi}_{year}")
