...
inputJson = 'nClimGrid_daily-input.json' ; # Update contents of this file to set your global_attributes
inputFilePath = '/global/cfs/projectdirs/m4581/obs4MIPs/obs4MIPs_input/NOAA-NCEI/nClimGrid-Daily/grids' 
inputVarName = ['prcp','tavg','tmax','tmin']
inputUnits = ['millimeter','degrees_Celsius','degrees_Celsius','degrees_Celsius']
outputVarName = ['pr','tas','tasmax','tasmin']
outputUnits = ['kg m-2 s-1','K','K','K']
unit_factor=[86400.,273.15,273.15,273.15]
...
for fi in range(len(inputVarName)): #looping over varaibles
    for year in range(1951, 2024):  # put the years you want to process here
        inputFiles = f"{inputFilePath}/{year}/ncdd-{year}??-grd-scaled.nc"
        if len(inputFiles) == 0:
            continue
        ...
        if inputUnits[fi] == 'millimeter':
            values = np.where(mask, cmor_missing, values/unit_factor[fi])
            valid_min = float(d.valid_min/unit_factor[fi])
            valid_max = float(d.valid_max/unit_factor[fi])
        elif inputUnits[fi] == 'degrees_Celsius':
            values = np.where(mask, cmor_missing, values+unit_factor[fi])
            valid_min = float(d.valid_min+unit_factor[fi])
            valid_max = float(d.valid_max+unit_factor[fi])
        else:
