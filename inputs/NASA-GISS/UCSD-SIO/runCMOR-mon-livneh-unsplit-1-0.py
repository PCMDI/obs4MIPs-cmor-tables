...
inputFilePath = '/global/cfs/projectdirs/m4581/obs4MIPs/obs4MIPs_input/UCSD-SIO/nonsplit_precip/processed_monthly' 
inputVarName = ['PRCP','Tmax','Tmin']
inputUnits = ['kg m-2 s-1','K','K']  #input units same as required (?) output units, no unit conversion needed
outputVarName = ['pr','tasmax','tasmin']
outputUnits = ['kg m-2 s-1','K','K']
valid_min=[0.,230.,230.]            #referenced from the actual input data values
valid_max=[0.002,330.,330.]         #referenced from the actual input data values
...
for fi in range(len(inputVarName)): #looping over varaibles
    inputFiles = f"{inputFilePath}/Livneh_Unsplit_monthly_{inputVarName[fi]}_1981-2005.nc"
    if len(inputFiles) == 0:
        continue
