...
inputFilePath = '/global/cfs/projectdirs/m4581/obs4MIPs/obs4MIPs_input/OSU/PRISM/daily' 
inputVarName = ['PPT','TMEAN','TMAX','TMIN']
inputUnits = ['mm','degreesC','degreesC','degreesC']  #input units same as required (?) output units, no unit conversion needed
outputVarName = ['pr','tas','tasmax','tasmin']
outputUnits = ['kg m-2 s-1','K','K','K']
unit_factor=[86400.,273.15,273.15,273.15]
valid_min=[0.,220.,220.,220.]            #referenced from the actual input data values
valid_max=[0.005,330.,330.,330.]         #referenced from the actual input data values
...
for fi in range(len(inputVarName)): #looping over varaibles
    for year in range(1981, 2022):  # put the years you want to process here
        inputFiles = f"{inputFilePath}/{inputVarName[fi]}/PRISM_{inputVarName[fi]}_{year}.nc"
        if len(inputFiles) == 0:
            continue
