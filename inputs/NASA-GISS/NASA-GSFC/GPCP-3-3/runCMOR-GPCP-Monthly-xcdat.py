...
for year in range(1983, 2025):  # put the years you want to process here
    inputFiles = glob.glob(f"{inputFilePath}/GPCPMON_L3_{year}??_V3.3.nc4")
    if len(inputFiles) == 0:
        continue
    ...
    cmor.set_variable_attribute(varid,'valid_min','f',float(d.valid_range[0]/sec))
    cmor.set_variable_attribute(varid,'valid_max','f',float(d.valid_range[-1]/sec))
