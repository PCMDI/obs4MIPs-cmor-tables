...
for year in range(1998, 2026):  # put the years you want to process here
   for month in range(1,13):
        inputFiles = glob.glob(f"{inputFilePath}/3B-DAY.MS.MRG.3IMERG.{year}{month:02}??-S000000-E235959.V07B.nc4")
        if len(inputFiles) == 0:
            continue
        ...
        # Due to CMOR warnings, time and time_bnds need to be rounded to be exact
        time = np.around(time)
        time_bnds = np.around(time_bnds)
